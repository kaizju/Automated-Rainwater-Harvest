...
function phLabel($v)
{
  return $v == 0 ? 'None' : ($v < 6.5 ? 'Low' : ($v > 8.5 ? 'High' : 'Optimal'));
}
function phColor($v)
{
  return ($v == 0 || ($v >= 6.5 && $v <= 8.5)) ? '#16a34a' : '#ef4444';
}
function turbLabel($v)
{
  return $v == 0 ? 'None' : ($v > 4 ? 'Poor' : ($v > 1 ? 'Moderate' : 'Excellent'));
}
function turbColor($v)
{
  return ($v == 0 || $v <= 1) ? '#16a34a' : ($v <= 4 ? '#d97706' : '#ef4444');
}
...
<body>
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <a href="<?php echo BASE_URL; ?>/App/Dashboard/dashboard.php" class="nav-item active">
    ...
    <a href="<?php echo BASE_URL; ?>/App/settings/settings.php" class="nav-item">
      <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      try {
        ...
        data.list.forEach(item => {
          ...
          if (!daily[key]) daily[key] = {
            name: new Date(item.dt * 1000).toLocaleDateString('en-US', {
              weekday: 'long'
            }),
            rain: [],
            chance: [],
