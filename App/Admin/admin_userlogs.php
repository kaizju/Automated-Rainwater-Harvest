...
<head>
    ...
    <style>
        /* ── BASE RESETS FOR THIS STANDALONE PAGE ── */
        /*
         * This page has no sidebar — it uses the top-nav pattern from all.css.
         * We reset the body flex + margin-left that all.css sets for sidebar layouts.
         */
        body {
            display: block;           /* override all.css flex (sidebar layout) */
            font-size: 13px;
            background: #f1f5f9;
            color: #374151;
        }
        ...
        /* ── CUSTOM CHECKBOX ──
         * Parent must be position:relative so ::after pins correctly.
         * The <input> is layered on top via z-index for click events.
         */
        .custom-checkbox {
            position: relative;
        ...
        .custom-checkbox input[type="checkbox"] {
            opacity: 0;
            position: absolute;
            inset: 0;           /* fills the wrapper exactly */
            z-index: 2;
            width: 100%;
            height: 100%;
            margin: 0;
        ...
        .btn-sm-action:hover  { opacity: .88; transform: translateY(-1px); }
        .btn-sm-action:active { transform: translateY(0); }
        ...
        @media (max-width: 480px) {
            .search-bar input { width: 110px; font-size: .78rem; }
            .btn-sm-action { padding: .38rem .65rem; font-size: .75rem; }
...
<!-- TOP NAV — uses .top-nav / .top-nav-brand / .top-nav-back from all.css -->
<nav class="top-nav">
...
<div class="content">
    ...
    <div class="table-wrapper">
                    ...
                    <?php if ($userRows): ?>
                        <?php foreach ($userRows as $u): ?>
                            <tr>
                                ...
                                <td>
                                    <!-- .role-badge / .role-admin / .role-user come from all.css -->
        ...
        <div class="tbl-footer">
            <div class="hint-text">
                Showing <b><?= count($userRows) ?></b> of <b><?= $totalUsers ?></b> users<?= $search ? ' — <em>' . htmlspecialchars($search) . '</em>' : '' ?>
            </div>
            <?php if ($totalPages > 1): ?>
                <div class="pager">
                    <?php if ($page > 1): ?>
...
$(function () {
    ...
    /* Bulk delete — prevent opening modal if nothing selected */
