    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        ...
        table.data-table th {
            ...
            white-space: nowrap;
            background: #fafafa; /* FIX 2: subtle header bg so it reads better */
        }
        table.data-table td {
            padding: .75rem 1.1rem;
            border-bottom: 1px solid #f3f4f6; /* FIX 3: consistent border color (was #f9fafb) */
        ...
        /* FIX 4: manager badge was missing */
        .role-manager { background: #ede9fe; color: #5b21b6; }

        .verified-icon   { color: #16a34a; font-size: 18px !important; vertical-align: middle; }
        .unverified-icon { color: #ef4444; font-size: 18px !important; vertical-align: middle; }
        ...
        .action-link {
            ...
            /* FIX 5: prevent Bootstrap anchor color override */
            color: inherit !important;
        }
        .action-link.edit   { color: #d97706 !important; }
        .action-link.edit:hover   { background: #fef3c7; }
        .action-link.delete { color: #ef4444 !important; }
        .action-link.delete:hover { background: #fef2f2; }
        ...
        /* ── CUSTOM CHECKBOX ── FIX 6: fully corrected implementation ── */
        .custom-checkbox {
            position: relative;
            display: inline-block; /* FIX: was block-level, now inline so it doesn't stretch td */
            width: 16px;
            height: 16px;
            vertical-align: middle;
        }
        ...
        /* FIX 7: checkmark ::after on label, not input — correct selector */
        ...
        .custom-checkbox input:checked + label::after {
            ...
            position: absolute;
            left: 5px;
            top: 2px;   /* FIX 8: was 3px — adjusted so tick sits centred in 16px box */
            width: 5px;
            height: 9px;
            border: solid #fff;
        ...
        .btn-sm-action {
            ...
            white-space: nowrap; /* FIX 9: prevent label wrapping on small screens */
        }
        .btn-green { background: #16a34a; color: #fff; }
        .btn-red   { background: #ef4444; color: #fff; }
        .btn-sm-action:hover  { opacity: .88; }
        .btn-sm-action:active { transform: scale(.97); }
        ...
        .modal .modal-header {
            ...
            /* FIX 10: override Bootstrap default border-bottom inside dark header */
            border-bottom: none;
        }
        .modal .modal-title {
            color: #fff;
        ...
        .modal .close {
            color: #fff;
            opacity: .75;
            text-shadow: none; /* FIX 11: Bootstrap adds dark text-shadow — removed */
        }
        ...
        .modal .modal-footer {
            /* FIX 12: Bootstrap adds top border that clashes with rounded corners */
            border-top: 1px solid #f3f4f6;
        }
        ...
        @media (max-width: 768px) {
            ...
            /* FIX 13: stack title-bar buttons on mobile without overflow */
            .table-title-bar { gap: .5rem; }
            .btn-sm-action { padding: .38rem .7rem; font-size: .78rem; }
        }
        @media (max-width: 480px) {
            ...
            table.data-table td:nth-child(5) { display: none; } /* hide Verified col */
        }
    </style>
</head>
<body>
...
<nav class="top-nav">
    <a href="<?php echo BASE_URL; ?>/App/Admin/admin_dashboard.php" class="top-nav-brand">
        <div class="logo-drop">💧</div>
        <span class="logo-name">EcoRain</span>
    </a>
    <a href="<?php echo BASE_URL; ?>/App/Admin/admin_dashboard.php" class="top-nav-back">
...
<div class="content">
    ...
    <div class="table-wrapper">
        <div class="table-title-bar">
            ...
            <!-- FIX 14: added action="#" + return false to prevent blank GET on empty search -->
            <form method="GET" class="search-bar" style="display:inline;" onsubmit="if(!this.q.value.trim())return false;">
                    ...
                    <?php if ($userRows): ?>
                        <?php foreach ($userRows as $u): ?>
                            <tr>
                                ...
                                <td>
                                    <!-- FIX 15: sanitize role value before using as CSS class -->
                                ...
                                <td style="white-space:nowrap;"><?= date('M j, Y', strtotime($u['created_at'])) ?></td>
                                <td>
        ...
        <div class="tbl-footer">
            <div class="hint-text">
                Showing <b><?= count($userRows) ?></b> of <b><?= $totalUsers ?></b> user<?= $totalUsers !== 1 ? 's' : '' ?>
                <?= $search ? ' — <em>' . htmlspecialchars($search) . '</em>' : '' ?>
            </div>
            <?php if ($totalPages > 1): ?>
                <div class="pager">
                    <?php if ($page > 1): ?>
                        <a href="?page=<?= $page-1 ?>&q=<?= urlencode($search) ?>">‹</a>
                    <?php endif; ?>
                    ...
                    <?php if ($logs): ?>
                        <?php foreach ($logs as $log): ?>
                            <tr>
                                ...
                                <td>
                                    <span class="status-<?= $log['status'] === 'success' ? 'ok' : 'bad' ?>">
                                        <?= ucfirst(htmlspecialchars($log['status'])) ?>
                                    </span>
                                </td>
                                <td><?= htmlspecialchars($log['ip_address'] ?? '—') ?></td>
                                <td style="white-space:nowrap;"><?= date('M j, H:i', strtotime($log['created_at'])) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="POST">
                ...
                <div class="modal-body">
                    <p>Delete <strong id="deleteUserEmail"></strong>? This action cannot be undone.</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Cancel</button>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="POST">
                ...
                <div class="modal-body">
                    <p>Delete <strong id="bulkDeleteCount">0</strong> selected user(s)? This cannot be undone.</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Cancel</button>
...
$(function () {
    ...
    // Bulk delete — guard: require at least one selection
