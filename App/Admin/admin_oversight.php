...
if ($filterUser)   { $where[] = '(u.email LIKE :user OR u.username LIKE :user2)';
                     $params[':user'] = '%'.$filterUser.'%'; $params[':user2'] = '%'.$filterUser.'%'; }
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nav-section-label { font-size: .6rem; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; color: #475569; padding: .65rem .75rem .25rem; }
    ...
    .t-btn { width: 34px; height: 34px; border: 1px solid var(--border); border-radius: 9px; background: var(--card-bg); display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--muted); position: relative; transition: border-color .15s; }
    ...
    .stat-num { font-family: 'Sora', sans-serif; font-size: 1.65rem; font-weight: 800; color: var(--text); line-height: 1; }
    .stat-lbl { font-size: .73rem; color: var(--muted); margin-top: .15rem; }
    .stat-card.critical .stat-num { color: #dc2626; }
    ...
    .filter-bar input, .filter-bar select { height: 34px; border: 1px solid var(--border); border-radius: 9px; background: var(--bg); color: var(--text); font-size: .8rem; font-family: 'DM Sans', sans-serif; padding: 0 .75rem; outline: none; transition: border-color .15s; }
    .filter-bar input:focus, .filter-bar select:focus { border-color: var(--accent); }
    .filter-bar input[type="text"] { width: 180px; }
    .btn-sm { height: 34px; padding: 0 .9rem; border-radius: 9px; font-size: .8rem; font-weight: 600; font-family: 'DM Sans', sans-serif; cursor: pointer; border: none; transition: background .15s; }
    ...
    .tbl th { font-size: .65rem; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; color: var(--subtle); padding: 0 .6rem .55rem 0; text-align: left; border-bottom: 1px solid var(--border); white-space: nowrap; }
    .tbl td { padding: .55rem .6rem .55rem 0; border-bottom: 1px solid #f8fafc; vertical-align: middle; color: #374151; }
    ...
    .action-chip { display: inline-flex; align-items: center; gap: .3rem; font-size: .75rem; font-weight: 500; color: var(--text); }

    /* USERS GRID */
    .users-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: .85rem; }
    .user-card { border: 1px solid var(--border); border-radius: 11px; padding: .9rem 1rem; display: flex; gap: .75rem; align-items: flex-start; transition: border-color .15s; }
    .user-card:hover { border-color: var(--accent); }
    .user-avatar { width: 38px; height: 38px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #fff; font-size: .78rem; font-weight: 700; flex-shrink: 0; }
    .user-name { font-size: .875rem; font-weight: 600; color: var(--text); }
    .user-email { font-size: .73rem; color: var(--muted); margin: .1rem 0 .35rem; }
    .user-meta { display: flex; gap: .4rem; align-items: center; flex-wrap: wrap; }
    .user-stat { font-size: .68rem; color: var(--subtle); }

    /* ALERT ITEMS */
    .alert-item { display: flex; gap: .75rem; align-items: flex-start; padding: .75rem 0; border-bottom: 1px solid #f8fafc; }
    .alert-item:last-child { border-bottom: none; }
    .alert-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; margin-top: .35rem; }

    /* PAGINATION */
    .pager { display: flex; gap: .35rem; align-items: center; margin-top: 1rem; flex-wrap: wrap; }
    .pager a, .pager span { display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 8px; font-size: .78rem; font-weight: 600; text-decoration: none; border: 1px solid var(--border); color: var(--muted); transition: background .12s, border-color .12s; }
    ...
    @media (max-width: 480px) { .stats-grid { grid-template-columns: 1fr; } .filter-bar { flex-direction: column; align-items: stretch; } .filter-bar input, .filter-bar select { width: 100%; } }
  </style>
</head>
<body>
...
<aside class="sidebar" id="sidebar">
  ...
  <a href="<?= BASE_URL ?>/App/Admin/admin_dashboard.php" class="nav-item <?= $activePage==='dashboard' ? 'active' : '' ?>">
  ...
  <a href="<?= BASE_URL ?>/App/Admin/admin_oversight.php" class="nav-item <?= $activePage==='oversight' ? 'active' : '' ?>">
  ...
  <a href="<?= BASE_URL ?>/App/Admin/admin_usage.php" class="nav-item <?= $activePage==='usage' ? 'active' : '' ?>">
    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
    Usage Stats
  </a>
  <a href="<?= BASE_URL ?>/App/Admin/admin_weather.php" class="nav-item <?= $activePage==='weather' ? 'active' : '' ?>">
  ...
  <a href="<?= BASE_URL ?>/App/Admin/admin_map.php" class="nav-item <?= $activePage==='map' ? 'active' : '' ?>">
  ...
  <a href="<?= BASE_URL ?>/App/Admin/admin_userlogs.php" class="nav-item <?= $activePage==='users' ? 'active' : '' ?>">
  ...
  <a href="<?= BASE_URL ?>/App/Admin/admin_settings.php" class="nav-item <?= $activePage==='settings' ? 'active' : '' ?>">
...
new Chart(document.getElementById('roleChart').getContext('2d'), {
  ...
  data: {
    ...
    datasets: [
      { label: 'Admin',   data: <?= json_encode($roleChartAdmin) ?>,   borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,.08)',  tension: .4, fill: true, borderWidth: 2, pointRadius: 3 },
      { label: 'Manager', data: <?= json_encode($roleChartManager) ?>, borderColor: '#8b5cf6', backgroundColor: 'rgba(139,92,246,.06)',  tension: .4, fill: true, borderWidth: 2, pointRadius: 3 },
      { label: 'User',    data: <?= json_encode($roleChartUser) ?>,    borderColor: '#10b981', backgroundColor: 'rgba(16,185,129,.06)', tension: .4, fill: true, borderWidth: 2, pointRadius: 3 },
    ]
  },
  options: {
    responsive: true, maintainAspectRatio: false,
    plugins: { legend: { labels: { font: { family: 'DM Sans', size: 11 }, color: '#64748b', boxWidth: 12, boxHeight: 4, borderRadius: 2, useBorderRadius: true } } },
    scales: {
